...
export function PatternsRun({ session, template, patient, preview = false }: Props) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const r = new TestRunner(template.config, session.ear, 'practice', session.ref_db_snapshot ?? undefined, curve)
      runnerRef.current = r
      if (!preview) {
  ...
  const tones = Object.keys(template.config.tones)
  ...
  if (!state) return <div className="p-8">Cargando evaluación...</div>
      ...
      <div className="p-8 max-w-5xl mx-auto">
        ...
        {(state.finished || allTestAnswered) && (
          <Card className="border-2 border-[var(--primary)]">
            <CardHeader>
              <CardTitle>{preview ? 'Resumen vista previa' : 'Finalizar evaluación'}</CardTitle>
            </CardHeader>
            <CardContent>
