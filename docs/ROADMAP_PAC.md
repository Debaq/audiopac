...
**Leyenda de estados** (convención para headings y bullets):

| Emoji | Significado |
|---|---|
| ✅ | Hecho / implementado |
| 🚧 | En curso |
| ⚠️ | Parcial — hecho con caveats / partes pendientes |
| 📝 | Pendiente / plan por ejecutar |
| ❌ | Bloqueado / descartado |

Convención: `## N. Título <emoji>` en headings y `- <emoji> ...` en bullets. El parser (`src/lib/roadmapParser.ts`) y el visor `/roadmap` leen estos marcadores.
...
### 2.3 Fuera de MVP — denoise espectral hecho; resto sin prioridad

- ✅ Denoise espectral STFT (Fase 4, `src/lib/audio/denoise.ts`)
- 📝 Posponer: RNNoise, dereverb → requieren WASM o sidecar nativo, sin caso clínico que lo justifique aún.
...
- ✅ Denoise espectral STFT (FFT radix-2 N=1024, Hann, hop 256, perfil ruido percentil 20, gate por bin ±6/−12 dB, smoothing freq/tiempo, iSTFT overlap-add) integrado en `processClip` tras HP y antes de VAD. Flag `denoise: true` default, configurable vía `DEFAULT_PROC` (`src/lib/audio/denoise.ts`, `src/lib/audio/recording.ts:447-455`). OFF por lote en importador Sharvard (audio pro)
...
##### 8.6.6.1 SRT — knobs por agregar ✅ hecho
...
##### 8.6.6.2 Dichotic Digits — knobs por agregar ✅ hecho
...
##### 8.6.6.3 Knobs generalizables (todos los engines) ✅ hecho (v1)
...
##### 8.6.6.4 Escape hatch JSON ✅ hecho
...
##### 8.6.6.5 Plan de implementación ⚠️ parcial (knobs motor-específicos HINT/Matrix/SSW pendientes)
...
#### 8.7.2 Normalización previa del MD ⚠️ parcial (leyenda + headings con status hechos; bullets y redundancias `✅ hecho` sin tocar)

- ✅ Leyenda de estados al inicio del MD (tabla emoji → significado).
- ✅ Todos los headings `##`/`###`/`####`/`#####` llevan status emoji (se agregó a los pocos que faltaban en §8.6.6.x y §11.4).
- 📝 No hecho (fuera de alcance mínimo): consolidar bullets a `- ✅ ...`, eliminar redundancia `✅ hecho`, uniformar tablas.
...
#### 8.7.5 Entrypoints ✅ hecho

- ✅ Item en menú lateral (`AppLayout`): "Roadmap".
- ✅ Atajo `Ctrl+K` (Spotlight) busca headings del roadmap y navega a la sección.
- ✅ Footer "N funciones en curso" en sidebar que linkea a `/roadmap?filter=parcial` (⚠️ partial + 🚧 in_progress agregados; 🚧 no se usa en práctica).
- 📝 Opcional sin prioridad: badge de ítems nuevos desde última visita.
...
### 11.4 Prioridades propuestas 📝 pendiente
